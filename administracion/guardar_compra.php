...
mysqli_query($link,"INSERT INTO `compras` (`codigo_com`, `comprobante_com`, `cliente_com`, `registro_com`, `usuario_com`) VALUES ('$cod', '$ven','$cli', '$registro', '$usuario');");	
	...
	{
		$c_procentajeX=$c_ley-(($por*$c_ley)/100);
		$soles=bcdiv($c_procentajeX*$_POST['cambio-1'],'1',1);
		$total=$soles*$can;
...
for($i=2;$i<=$nitem;$i++)
{
	...
	{
		$c_procentajeAX=$c_leyA-(($porc*$c_leyA)/100);
		$soles=bcdiv($c_procentajeAX*$_POST['cambio-'.$i],'1',1);
		$totalA=$soles*$cant;
